...
def exportovat() -> None:
    ...
    for i, u in enumerate(User.get_all()):
        ...
        ws1.cell(i+2,5,value=u.prijmeni)
        ws1.cell(i+2,6,value=u.adresa)
        ws1.cell(i+2,7,value=u.telcislo)
        ws1.cell(i+2,8,value=u.mail_rodicu)
        ws1.cell(i+2,9,value=u.odbornost)
        ws1.cell(i+2,10,value=pretty_date(u.datum_narozeni))
        ...
        ws1.cell(i+2,16,value=u.uzamcene_zmeny_udaju)
        ws1.cell(i+2,17,value=u.uzamcene_zmeny_callu)
        ws1.cell(i+2,18,value=u.uzamcene_zmeny_prace)
        ws1.cell(i+2,19,value=u.alergie)
        ws1.cell(i+2,20,value=u.skola)
        ws1.cell(i+2,21,value=pretty_datetime(u.datum_registrace))
        ws1.cell(i+2,22,value=u.odevzdany_motivacni_dotaznik)
        ws1.cell(i+2,23,value=u.osloveni_1p)
        ws1.cell(i+2,24,value=u.osloveni_5p)
        ws1.cell(i+2,25,value=u.zajmeno)
