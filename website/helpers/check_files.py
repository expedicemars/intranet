...
def check_prubeh_rocniku() -> None:
    ...
    if path.exists():
        ...
        path.touch()
        with open(path, "w") as file:
            file.write(json.dumps({"datum_konce_registrace":str(date.today()),"datum_zacatku_registrace":str(date.today()),"otevrena_registrace":False,"viditelna_zadani":False, "koordinator_internetovych_kol":"", "info_o_konferenci_viditelne": False, "hodiny_motivacni_call": 3, "aktualni_faze": "otevrena_registrace"}, indent=4))
