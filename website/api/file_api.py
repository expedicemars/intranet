...
from website.paths import user_data_folder_path, zadani_folder_path, prohlaseni_path, exporty_path, sablony_folder_path, vzorove_vypracovani_path, mail_logo_path, harmonogram_path
...
@file_api.route("/send_filename_vlastniho_shrnuti")
@require_role_on_current_user("user")
def send_filename_vlastniho_shrnuti():
    return get_shrnuti_filename(current_user.id)

@file_api.route("/send_filename_ciziho_shrnuti/<int:id>")
@require_role_on_current_user("editing_users_allowed")
def send_filename_ciziho_shrnuti(id):
    return get_shrnuti_filename(id)

@file_api.route("/vlastni_shrnuti/<string:name>")
@require_role_on_current_user("user")
def vlastni_shrnuti(name):
    p = user_data_folder_path() / str(current_user.id) / name
    return send_file(p)

@file_api.route("/cizi_shrnuti/<int:id>/<string:name>")
@require_role_on_current_user("editing_users_allowed")
def cizi_shrnuti(id, name: str):
        p = user_data_folder_path() / str(id) / name
        return send_file(p)
...
@file_api.route("/mail_logo")
def mail_logo():
    return send_file(mail_logo_path())

@file_api.route("/harmonogram_svg")
def harmonogram_svg():
    return send_file(harmonogram_path())