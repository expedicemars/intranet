...
def home():
    return render_template("home.html", roles = get_access_rights(current_user), otevrena_registrace = get_registrace_otevrena(), user_progress=current_user.progress)


@default_views.route("/nahlasit_bug", methods=["GET", "POST"])
def nahlasit_bug():
    if request.method == "GET":
        return render_template("nahlasit_chybu.html", roles = get_access_rights(current_user), user_progress=current_user.progress)
    else:
...
def known_bugs():
    return render_template("zname_chyby.html", roles = get_access_rights(current_user), user_progress=current_user.progress)